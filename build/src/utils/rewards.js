...
exports.IsolatedRewardTelegram = exports.createIsolatedRewardTelegram = exports.LinkRewardTelegram = exports.createLinkRewardTelegram = exports.ReferralRewardTelegram = exports.createReferralRewardTelegram = exports.SignUpRewardTelegram = exports.createSignUpRewardTelegram = void 0;
 ...
 */
async function createSignUpRewardTelegram(eventId, userTelegramID, responsePath, userHandle, userName, patchwallet, tokenAddress, chainName) {
    const reward = new SignUpRewardTelegram(eventId, userTelegramID, responsePath, userHandle, userName, patchwallet, tokenAddress, chainName);
    if (!(await reward.initializeRewardDatabase()))
        return false;
    return reward;
}
exports.createSignUpRewardTelegram = createSignUpRewardTelegram;
/**
 * Represents a Telegram sign-up reward.
 */
class SignUpRewardTelegram {
    eventId;
    userTelegramID;
    responsePath;
    userHandle;
     ...
     */
    constructor(eventId, userTelegramID, responsePath, userHandle, userName, patchwallet, tokenAddress, chainName) {
    ...
    async initializeRewardDatabase() {
        ...
        if (await this.getOtherRewardFromDatabase())
            return false;
        if (this.tx) {
    ...
    async getRewardFromDatabase() {
        return await this.db.collection(constants_1.REWARDS_COLLECTION).findOne({
    ...
    async updateInDatabase(status, date) {
        await this.db.collection(constants_1.REWARDS_COLLECTION).updateOne({
            eventId: this.eventId,
            reason: this.reason,
            userTelegramID: this.userTelegramID,
        }, {
        ...
        console.log(`[${this.eventId}] sign up reward for ${this.userTelegramID} in MongoDB as ${status} with transaction hash : ${this.txHash}.`);
    }
    /**
    ...
    async saveToFlowXO() {
        ...
        await axios_1.default.post(secrets_1.FLOWXO_NEW_SIGNUP_REWARD_WEBHOOK, {
            userTelegramID: this.userTelegramID,
            responsePath: this.responsePath,
            walletAddress: this.patchwallet,
            reason: this.reason,
    ...
    async sendTx() {
        try {
            ...
            return await (0, patchwallet_1.sendTokens)(secrets_1.SOURCE_TG_ID, this.patchwallet, this.amount, await (0, patchwallet_1.getPatchWalletAccessToken)(), this.tokenAddress, this.chainName);
        }
        catch (error) {
            // Log error if sending tokens fails
            console.error(`[${this.eventId}] sign up reward for ${this.userTelegramID} - Error processing PatchWallet token sending: ${error}`);
            return false;
        }
    }
}
exports.SignUpRewardTelegram = SignUpRewardTelegram;
/**
 ...
 */
async function createReferralRewardTelegram(eventId, userTelegramID, responsePath, userHandle, userName, patchwallet, tokenAddress, chainName) {
    // Create a new instance of ReferralRewardTelegram
    const reward = new ReferralRewardTelegram(eventId, userTelegramID, responsePath, userHandle, userName, patchwallet, tokenAddress, chainName);
    // Initialize the reward database by connecting and retrieving necessary information
    await reward.initializeRewardDatabase();
    return reward;
}
exports.createReferralRewardTelegram = createReferralRewardTelegram;
/**
 * Represents a Telegram referral reward.
 */
class ReferralRewardTelegram {
    eventId;
    userTelegramID;
    responsePath;
    userHandle;
    ...
    async getTransfersFromDatabase() {
        return this.db.collection(constants_1.TRANSFERS_COLLECTION).find({
    ...
    async setParentTx() {
        ...
        if (!this.parentTx) {
            console.log(`[${this.eventId}] no referral reward to distribute with ${this.userTelegramID} as a new user.`);
            return false;
        }
        return true;
    }
    ...
    async getReferent() {
        try {
            this.referent = await this.db.collection(constants_1.USERS_COLLECTION).findOne({
                userTelegramID: this.parentTx.senderTgId,
            });
            if (!this.referent) {
                console.log(`[${this.eventId}] sender ${this.parentTx.senderTgId} who is supposed to receive a referral reward is not a user.`);
            }
            return Boolean(this.referent);
        }
        catch (error) {
            console.error(`[${this.eventId}] Error trying to get referent information: ${error}`);
            return false;
        }
    }
    /**
    ...
    async getRewardSameFromDatabase() {
        this.tx = await this.db.collection(constants_1.REWARDS_COLLECTION).findOne({
            eventId: this.eventId,
            reason: this.reason,
        });
        if (this.tx) {
    ...
    async getRewardFromDatabaseWithOtherEventId() {
        return Boolean(await this.db.collection(constants_1.REWARDS_COLLECTION).findOne({
            eventId: { $ne: this.eventId },
            reason: this.reason,
    ...
    async updateReferentWallet() {
        this.referent.patchwallet =
            this.referent?.patchwallet ??
                (await (0, patchwallet_1.getPatchWalletAddressFromTgId)(this.referent.userTelegramID));
    }
    /**
    ...
    async updateInDatabase(status, date) {
        ...
        console.log(`[${this.eventId}] referral reward for ${this.referent.patchwallet} sending tokens to ${this.patchwallet} in ${this.parentTx.transactionHash} in MongoDB as ${status} with transaction hash : ${this.txHash}.`);
    }
    /**
     * Saves referral reward transaction information to FlowXO.
     */
    async saveToFlowXO() {
        await axios_1.default.post(secrets_1.FLOWXO_NEW_REFERRAL_REWARD_WEBHOOK, {
    ...
    async sendTx() {
        try {
            return await (0, patchwallet_1.sendTokens)(secrets_1.SOURCE_TG_ID, this.referent.patchwallet, this.amount, await (0, patchwallet_1.getPatchWalletAccessToken)(), this.tokenAddress, this.chainName);
        }
        catch (error) {
            console.error(`[${this.eventId}] Error processing PatchWallet referral reward for ${this.referent.patchwallet}: ${error}`);
            return false;
        }
    }
}
exports.ReferralRewardTelegram = ReferralRewardTelegram;
/**
 ...
 */
async function createLinkRewardTelegram(eventId, userTelegramID, referentUserTelegramID, tokenAddress, chainName) {
    const reward = new LinkRewardTelegram(eventId, userTelegramID, referentUserTelegramID, tokenAddress, chainName);
    if (!(await reward.initializeRewardDatabase()))
        return false;
    return reward;
}
exports.createLinkRewardTelegram = createLinkRewardTelegram;
/**
 * Represents a link reward for Telegram users.
 */
class LinkRewardTelegram {
    eventId;
    userTelegramID;
     ...
     * @param {string} chainName - The blockchain name.
     */
    constructor(eventId, userTelegramID, referentUserTelegramID, tokenAddress, chainName) {
        // Properties related to user and reward details
        this.eventId = eventId;
        this.userTelegramID = userTelegramID;
    ...
    async initializeRewardDatabase() {
        ...
        if (!(await this.getReferent())) {
            console.log(`[${this.eventId}] ${this.referentUserTelegramID} referent user is not a user to process the link reward.`);
            return false;
        }
        this.tx = await this.getRewardFromDatabase();
        if (await this.getOtherRewardFromDatabase())
    ...
    async getReferent() {
        ...
        this.referent.patchwallet =
            this.referent?.patchwallet ??
                (await (0, patchwallet_1.getPatchWalletAddressFromTgId)(this.referentUserTelegramID));
    ...
    async updateInDatabase(status, date) {
        ...
        console.log(`[${this.eventId}] link for ${this.referentUserTelegramID} sponsoring ${this.userTelegramID} in MongoDB as ${status} with transaction hash : ${this.txHash}.`);
    }
    /**
     * Checks if the treatment duration has exceeded the limit.
     * @returns {Promise<boolean>} - True if the treatment duration has exceeded, false otherwise.
    ...
    async saveToFlowXO() {
        // Send transaction information to FlowXO
        await axios_1.default.post(secrets_1.FLOWXO_NEW_LINK_REWARD_WEBHOOK, {
            userTelegramID: this.referentUserTelegramID,
            responsePath: this.referent.responsePath,
            walletAddress: this.referent.patchwallet,
            reason: this.reason,
    ...
    async sendTx() {
        try {
            ...
            // Log error if sending tokens fails
            console.error(`[${this.eventId}] link reward for ${this.referentUserTelegramID} - Error processing PatchWallet token sending: ${error}`);
            return false;
        }
    }
}
exports.LinkRewardTelegram = LinkRewardTelegram;
/**
 ...
 */
async function createIsolatedRewardTelegram(eventId, userTelegramID, reason, amount, message, responsePath, userHandle, userName, patchwallet, tokenAddress, chainName) {
    const reward = new IsolatedRewardTelegram(eventId, userTelegramID, reason, amount, message, responsePath, userHandle, userName, patchwallet, tokenAddress, chainName);
    if (!(await reward.initializeRewardDatabase()))
        return false;
    return reward;
}
exports.createIsolatedRewardTelegram = createIsolatedRewardTelegram;
class IsolatedRewardTelegram {
    eventId;
    userTelegramID;
    responsePath;
    userHandle;
     ...
     */
    constructor(eventId, userTelegramID, reason, amount, message, responsePath, userHandle, userName, patchwallet, tokenAddress, chainName) {
        this.eventId = eventId;
        this.userTelegramID = userTelegramID;
        this.responsePath = responsePath;
        this.userHandle = userHandle;
    ...
    async initializeRewardDatabase() {
        ...
        this.patchwallet =
            this.patchwallet ??
                (await (0, patchwallet_1.getPatchWalletAddressFromTgId)(this.userTelegramID));
        this.tx = await this.getRewardFromDatabase();
        if (await this.getOtherRewardFromDatabase())
            return false;
        if (this.tx) {
    ...
    async saveToFlowXO() {
        // Send transaction information to FlowXO
        await axios_1.default.post(secrets_1.FLOWXO_NEW_ISOLATED_REWARD_WEBHOOK, {
            userTelegramID: this.userTelegramID,
            responsePath: this.responsePath,
            walletAddress: this.patchwallet,
            reason: this.reason,
...
exports.IsolatedRewardTelegram = IsolatedRewardTelegram;
