...
const generateWorkflow = ({ address, webhook, responsepath, trigger, chains, creator, }) => {
    const workflow = {
        title: `[Grindery AI] PatchWallet ${address.toLowerCase()} ${trigger === 'evmWallet' ? 'native token' : 'erc-20 token'} transaction to webhook`,
        trigger: {
            type: 'trigger',
            connector: trigger,
    ...
    if (trigger === 'evmWallet') {
        ...
        workflow.actions[0].input.data.blockNumber =
            '{{trigger.blockNumber}}';
        workflow.actions[0].input.data.txfees = '{{trigger.txfees}}';
    }
    if (trigger === 'erc20') {
        workflow.actions[0].input.data.hash =
    ...
    workflow.actions[0].input.data = JSON.stringify(workflow.actions[0].input.data);
    return workflow;
};
exports.generateWorkflow = generateWorkflow;
//# sourceMappingURL=generateWorkflow.js.map