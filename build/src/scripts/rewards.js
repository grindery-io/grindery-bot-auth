...
exports.distributeReferralRewards = exports.distributeSignupRewards = void 0;
...
async function distributeSignupRewards() {
    ...
    try {
        ...
        const rewardsCollection = db.collection(constants_1.REWARDS_COLLECTION);
        // Initialize Patch Wallet access token and last renewal time
        let patchWalletAccessToken = await (0, patchwallet_1.getPatchWalletAccessToken)();
        ...
        for (const user of allUsers) {
            ...
            if (!allRewards.some((reward) => reward.userTelegramID === user.userTelegramID)) {
                console.log(`[${userCount}/${allUsers.length}] User ${user.userTelegramID} has no sign up reward`);
                // Manage token renewal at intervals
                if (Date.now() - lastTokenRenewalTime >= 50 * 60 * 1000) {
                    patchWalletAccessToken = await (0, patchwallet_1.getPatchWalletAccessToken)();
                ...
                const userPromise = (async () => {
                    try {
                        ...
                        const txReward = await (0, patchwallet_1.sendTokens)(secrets_1.SOURCE_TG_ID, user.patchwallet, '100', patchWalletAccessToken);
                        ...
                        if (txReward.data.txHash) {
                            ...
                            console.log(`[${userCount}/${allUsers.length}] User ${user.userTelegramID} has been rewarded for signing up.`);
                        }
                    }
                    catch (error) {
...
async function distributeReferralRewards() {
    ...
    try {
        ...
        const allTransfers = await transfersCollection
            .find({ status: constants_1.TRANSACTION_STATUS.SUCCESS })
            .sort({ dateAdded: 1 })
            .toArray();
        const allRewards = await rewardsCollection
            .find({
        ...
        for (const user of allUsers) {
            ...
            const firstValidTransfer = allTransfers.find((transfer) => transfer.recipientTgId === user.userTelegramID &&
                transfer.senderTgId !== user.userTelegramID &&
                new Date(transfer.dateAdded) < new Date(user.dateAdded));
            if (!firstValidTransfer)
                continue;
            // Check for existing rewards for this user
            const existingReward = allRewards.some((reward) => reward.parentTransactionHash === firstValidTransfer.transactionHash ||
                web3_1.default.utils.toChecksumAddress(reward.newUserAddress) ===
                    web3_1.default.utils.toChecksumAddress(user.patchwallet));
            if (existingReward)
                continue;
            console.log(`[${userCount}/${allUsers.length}] User ${firstValidTransfer.senderTgId} has no referral reward for sending tokens to ${user.userTelegramID}.`);
            const rewardPromise = (async () => {
                try {
                    // Determine reward details
                    const rewardWallet = await (0, patchwallet_1.getPatchWalletAddressFromTgId)(firstValidTransfer.senderTgId);
                    const rewardAmount = new Date(firstValidTransfer.dateAdded) <
                    ...
                    const txReward = await (0, patchwallet_1.sendTokens)(secrets_1.SOURCE_TG_ID, rewardWallet, rewardAmount, patchWalletAccessToken);
                    await rewardsCollection.insertOne({
                        eventId: (0, uuid_1.v4)(),
                        ...
                        dateAdded: new Date(),
                        newUserAddress: user.patchwallet ??
                            (await (0, patchwallet_1.getPatchWalletAddressFromTgId)(user.userTelegramID)),
                        status: constants_1.TRANSACTION_STATUS.SUCCESS,
                    });
                    console.log(`User ${firstValidTransfer.senderTgId} has been rewarded for sending tokens to ${user.userTelegramID}.`);
                }
                catch (error) {
                    console.error('An error occurred during reward distribution:', error);
                }
...
exports.distributeReferralRewards = distributeReferralRewards;
...
async function saveRewards(rewards) {
    ...
    const formattedMissingRewards = rewards
        ...
        return {
            userTelegramID: '',
            responsePath: '',
            walletAddress: web3_1.default.utils.toChecksumAddress(rewards.to),
    ...
    const walletAddresses = formattedMissingRewards.map((reward) => reward.walletAddress);
    ...
    formattedMissingRewards.forEach((reward) => {
        const matchingUser = userData.find((user) => user.patchwallet === reward.walletAddress);
        if (matchingUser) {
...
async function updateRewardMessages() {
    const db = await conn_1.Database.getInstance();
    const collection = db.collection(constants_1.REWARDS_COLLECTION);
    ...
    for (const reward of rewards) {
        ...
        if (updatedMessage) {
            ...
            processedCount++;
            console.log(`[${processedCount}/${totalRewards}]: Message updated for ${reward.userTelegramID}`);
        }
    }
...
async function updateRewardsStatus() {
    try {
        ...
        if (bulkWriteOperations.length > 0) {
            ...
            console.log(`Updated ${result.modifiedCount} rewards with status: success`);
        }
        else {
...
async function importMissingRewardsFromCSV(fileName) {
    ...
    fs_1.default.createReadStream(fileName)
        ...
        const existingHashes = await rewardsCollection
            ...
            .toArray();
        const missingRewards = rewards.filter((reward) => !existingHashes.includes(reward.transaction_hash));
        console.log('Number of rewards in csv file: ', rewards.length);
        console.log('Number of rewards in `rewards` collection: ', existingHashes.length);
        console.log('Number of missing rewards in `rewards` collection: ', missingRewards.length);
        let count = 1;
        for (const reward of missingRewards) {
            console.log(`Formatting reward index: ${count} - total: ${missingRewards.length}`);
            const senderUser = await usersCollection.findOne({
                walletAddress: web3_1.default.utils.toChecksumAddress(reward.from_address),
            });
...
async function checkMissingRewards(fileName) {
    ...
    fs_1.default.createReadStream(fileName)
        ...
        const hashesNotInDb = [...hashesInCsv].filter((hash) => !rewardsHashesInDb.includes(hash));
        if (hashesNotInDb.length === 0) {
            console.log('All rewards in CSV are present in the MongoDB collection.');
        }
        else {
            console.log("The following transaction hashes from the CSV aren't present in MongoDB rewards collection:");
...
async function updateParentTransactionHash() {
    try {
        ...
        for (const reward of allRewards) {
            ...
            const correspondingTransfer = allTransfers.find((e) => e.TxId === reward.parentTransactionHash);
...
async function calculateAverageRewardAmount() {
    try {
        // Connect to the database
        const db = await conn_1.Database.getInstance();
        const rewardsCollection = db.collection(constants_1.REWARDS_COLLECTION); // Make sure to use the correct collection
        ...
        console.error('An error occurred while calculating the average reward amount:', error);
    }
    finally {
        // Exit the script
        process.exit(0);
...
async function getTransactionsAndRecipientsFromId(userId) {
    try {
        ...
        for (let i = 0; i < uniqueSponsoredUserIDs.length; i++) {
            ...
            const transactionsDone = allTransfers.filter((transfer) => transfer.senderTgId === userID).length;
            const distinctRecipients = new Set(allTransfers
...
async function cleanupRewardsDB() {
    try {
        ...
        if (rewardsToUpdate.length > 0) {
            ...
            const bulkOps = await Promise.all(rewardsToUpdate.map(async (reward, index) => {
        ...
        const result = await rewardsCollection.deleteMany({
            walletAddress: secrets_1.SOURCE_WALLET_ADDRESS,
        });
        console.log(`${result.deletedCount} reward documents to our source wallet have been deleted.`);
    }
    catch (error) {
        console.error('An error occurred:', error);
    }
