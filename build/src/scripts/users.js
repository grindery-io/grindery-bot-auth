...
async function importUsersFromCSV(filePath) {
    ...
    try {
        ...
        fs_1.default.createReadStream(filePath)
            ...
            const existingTelegramIDs = (await collection.distinct('userTelegramID')).map(String);
            // Filter out new users not present in the database
            const newUsers = toInsert.filter((entry) => !existingTelegramIDs.includes(entry.userTelegramID));
...
async function importUsersFromJSON(filePath) {
    try {
        const db = await conn_1.Database.getInstance();
        const collection = db.collection(constants_1.USERS_COLLECTION);
        const existingUserIds = new Set((await collection.distinct('userTelegramID')).map(String));
        ...
        const newUsers = parsedObjects.filter((newUser) => !existingUserIds.has(newUser.userTelegramID));
        if (newUsers.length > 0) {
            await collection.insertMany(newUsers);
...
async function removeUsersScientificNotationInTelegramID() {
    try {
        ...
        console.log(`Deleted ${deleteResult.deletedCount} users with '+' in userTelegramID`);
    }
    catch (error) {
...
async function getUsersWithoutOutgoingTransfersAndExportToCSV() {
    try {
        ...
        const senderTgIdsSet = new Set(allTransfers.map((transfer) => transfer.senderTgId));
        ...
        if (usersWithoutOutgoingTransfers.length > 0) {
            const csvWriterObject = (0, csv_writer_1.createObjectCsvWriter)({
            ...
            console.log(`Users have been exported to users_without_outgoing_transfers.csv`);
        }
        else {
...
async function exportUsersWithHandleEndingInDigitsToCSV() {
    try {
        ...
        if (usersWithHandleEndingInDigits.length > 0) {
            ...
            console.log(`Users with handle ending in digits exported to users_with_handle_ending_in_digits.csv`);
        }
        else {
