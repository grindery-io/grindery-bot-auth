...
const telegramPromise_1 = tslib_1.__importDefault(require("../utils/telegramPromise"));
const uuid_1 = require("uuid");
const telegramClient_1 = tslib_1.__importDefault(require("../utils/telegramClient"));
 ...
 * }
 */
router.post('/init', auth_1.isRequired, async (req, res) => {
    const operationId = (0, uuid_1.v4)();
    const client = (0, telegramClient_1.default)(new index_1.StringSession(''));
    operations[operationId] = {
        status: 'pending',
        client: client,
        phoneCodePromise: null,
    };
    const globalPhoneCodePromise = (0, telegramPromise_1.default)();
    ...
    client
        ...
        phoneCode: async () => {
            if (operations[operationId].phoneCodePromise) {
                ...
                operations[operationId].phoneCodePromise = (0, telegramPromise_1.default)();
                return code;
            }
 ...
 * }
 */
router.post('/callback', auth_1.isRequired, async (req, res) => {
 ...
 * }
 */
router.get('/status', auth_1.isRequired, async (req, res) => {
    const client = (0, telegramClient_1.default)(new index_1.StringSession(req.query.session));
 ...
 * }
 */
router.get('/contacts', auth_1.isRequired, async (req, res) => {
    const client = (0, telegramClient_1.default)(new index_1.StringSession(req.query.session));
    await client.connect();
    if (!client.connected) {
        return res.status(200).json([]);
    }
    const contacts = await client.invoke(new telegram_1.Api.contacts.GetContacts({
