...
const grindery_nexus_client_1 = tslib_1.__importDefault(require("grindery-nexus-client"));
const generateWorkflow_1 = require("../utils/generateWorkflow");
const secrets_1 = require("../../secrets");
const NexusClient = grindery_nexus_client_1.default;
...
router.post('/wallet', async (req, res) => {
    ...
    try {
        ...
        const getAccessToken = await axios_1.default.post('https://orchestrator.grindery.org/oauth/token', {
            refresh_token: secrets_1.GRINDERY_ACCOUNT_REFRESH_TOKEN,
        ...
        if (!address) {
            const patchWalletResponse = await axios_1.default.post('https://paymagicapi.com/v1/resolver', {
                userIds: `tel:${req.body.phone}`,
            });
            address = patchWalletResponse.data.users[0].accountAddress;
        }
        // generate workflows
        const walletWorkflow = (0, generateWorkflow_1.generateWorkflow)({
            address,
        ...
        const erc20Workflow = (0, generateWorkflow_1.generateWorkflow)({
            address,
            webhook: req.body.webhook,
            responsepath: req.body.responsepath,
            trigger: 'erc20',
        ...
        const workflows = await client.workflow.list({
            workspaceKey: secrets_1.GRINDERY_ACCOUNT_WORKSPACE_KEY,
        });
        const walletWorkflowExists = workflows.find((workflow) => workflow.workflow.title === walletWorkflow.title);
        const erc20WorkflowExists = workflows.find((workflow) => workflow.workflow.title === erc20Workflow.title);
...
router.post('/webhook', async (_req, res) => {
    res.redirect(307, secrets_1.WALLET_NOTIFICATION_WEBHOOK_URL);
});
