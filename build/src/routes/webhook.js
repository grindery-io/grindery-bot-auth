...
const topicName = secrets_1.PUBSUB_TOPIC_NAME;
const subscriptionName = secrets_1.PUBSUB_SUBSCRIPTION_NAME;
const router = express_1.default.Router();
router.get('/unacked-messages', auth_1.authenticateApiKey, async (_req, res) => {
    try {
        const client = new monitoring_1.MetricServiceClient();
        const timeSeries = await client.listTimeSeries({
            name: client.projectPath(secrets_1.PROJECT_ID),
            filter: `metric.type="pubsub.googleapis.com/subscription/num_unacked_messages_by_region" AND resource.labels.subscription_id="${secrets_1.PUBSUB_SUBSCRIPTION_NAME}"`,
            interval: {
    ...
    try {
        if (req.body.event === 'new_transaction_batch') {
            req.body.params = req.body.params.filter((transaction) => {
                return (/^\d+$/.test(transaction.amount) && parseInt(transaction.amount) > 0);
            });
        ...
        if (req.body.event === 'new_transaction') {
            if (!/^\d+$/.test(req.body.params.amount) ||
        ...
        req.body.event === 'new_transaction_batch'
            ? req.body.params.forEach((transaction) => (transaction.eventId = (0, uuid_1.v4)()))
            : (req.body.params.eventId = (0, uuid_1.v4)());
...
const listenForMessages = () => {
    ...
    const subscription = pubSubClient.subscription(subscriptionName, {
        minAckDeadline: pubsub_1.Duration.from({
            millis: parseInt(secrets_1.PUBSUB_MIN_ACK_DEADLINE, 10) || 60 * 1000,
        }),
        maxAckDeadline: pubsub_1.Duration.from({
            millis: parseInt(secrets_1.PUBSUB_MAX_ACK_DEADLINE, 10) || 1200 * 1000,
        }),
        flowControl: {
            maxMessages: parseInt(secrets_1.PUBSUB_CONCURRENCY, 10) || 50,
        },
    });
    ...
    const messageHandler = async (message) => {
        ...
        if ((message.deliveryAttempt || 0) > 2 &&
        ...
        try {
            ...
            switch (messageData.event) {
                ...
                case 'new_transaction':
                    processed = await (0, transaction_1.handleNewTransaction)(messageData.params);
                    break;
                ...
                case 'new_transaction_batch':
                    for (const singleTransaction of messageData.params) {
                        ...
                        const transactionDataBuffer = Buffer.from(JSON.stringify(transactionEvent));
                        await pubSubClient
                            .topic(topicName)
                ...
                case 'new_reward':
                    processed = await (0, webhook_1.handleNewReward)(messageData.params);
                    break;
                // Isolated reward
                case 'isolated_reward':
                    processed = await (0, isolated_reward_1.handleIsolatedReward)(messageData.params);
                    break;
                case 'swap':
                    processed = await (0, swap_1.handleSwap)(messageData.params);
                    break;
                default:
                    processed = true;
                    break;
            ...
            console.log('Acknowledged message:', JSON.stringify(messageData, null, 2));
