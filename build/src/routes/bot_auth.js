...
router.get('/init', async (req, res) => {
    try {
        ...
        res.redirect(`https://orchestrator.grindery.org/oauth/authorize/?redirect_uri=${encodeURIComponent(`${currentDomain}/v1/bot-auth/callback`)}&response_type=code&state=${encodedState}`);
    }
    catch (error) {
...
router.get('/callback', async (req, res) => {
    try {
        ...
        const tokenResponse = await axios_1.default.post('https://orchestrator.grindery.org/oauth/token', {
            code: req.query.code,
        ...
        const userPropsResponse = await axios_1.default.post('https://orchestrator.grindery.org', {
            jsonrpc: '2.0',
        ...
        const decodeState = JSON.parse(Buffer.from(String(req.query.state), 'base64url').toString('utf-8'));
        if (!userPropsResponse.data.result ||
            !userPropsResponse.data.result.email) {
            return res.status(400).json({ error: 'Email is missing' });
        }
        if (userPropsResponse.data.result.patchwallet_telegram &&
        ...
        const updateUserResponse = await axios_1.default.post('https://orchestrator.grindery.org', {
            jsonrpc: '2.0',
...
//# sourceMappingURL=bot_auth.js.map